...
(function () {
    ...
    var generator = {
        generate: function (aDataBlitz, aTuple) {
            if (aDataBlitz.configuration.constant)
                return aDataBlitz.configuration.constant
            else if ((aDataBlitz.configuration.likelyhoods))
                return chance.weighted(aDataBlitz.configuration.possibleValues, aDataBlitz.configuration.likelyhoods);
            else if ((aDataBlitz.configuration.max))
                return chance.natural({min: aDataBlitz.configuration.min, max: aDataBlitz.configuration.max});
            else
                return 0;
        }
    }
